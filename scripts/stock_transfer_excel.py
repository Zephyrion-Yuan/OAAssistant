...
def run(input_path):
    ...
    for row in rows:
        ...
        material_name = normalize_cell(main_ws.cell(row=row, column=columns.get("materialName", 0)).value) if columns.get("materialName") else ""
    ...
    material_plans = [
        {
            "materialCode": material_code,
            "materialName": material_names.get(material_code, ""),
            "quantity": decimal_text(quantity),
            "unit": material_units.get(material_code, ""),
        }
...
def main():
    parser = argparse.ArgumentParser(description="Extract workflow 89 stock transfer input data from a purchase workbook.")
