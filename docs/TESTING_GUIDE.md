...
1. **配置抽屉 → WBS 管理**(点右上「配置」):新增一条 WBS,例如
   - WBS编码 `C2-0225002.06.01`、别称 `传感器项目, SA探针`、需求工厂代码 `1010`、成本中心 `CC-1010-01`、采购人 `ZN092-张三`、库存地点名称 `实验室仓`、库存地点SAP `H001` → 保存。
...
### 3.3 备好 WBS 主数据
配置抽屉 → WBS 管理:确保用到的每个 WBS 都维护了 **成本中心(412 用)/ 库存地点(89、414 用)/ 采购人 + 送货地址(458 附件用)/ 别称(可选)**。否则对应草稿会被「跳过 needsInput」。
