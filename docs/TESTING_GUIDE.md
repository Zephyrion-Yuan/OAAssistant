...
### 3.3 备好 WBS 主数据
配置抽屉 → WBS 管理:确保用到的每个 WBS 都维护了 **成本中心(412 用)/ 默认库存地点(89 转储按转出、转入各自 WBS 读取；414 入库读取入库 WBS)/ 采购人 + 送货地址 + 采购类型(458 用)/ 别称(可选)**。`是否为项目型`、`采购类型`、`附件需求类型`、`库存地点SAP/名称` 来自后端选项目录。否则对应草稿会停在可续跑的 `needs_input`，用户可直接补 WBS、成本中心或库存地点后原位续跑。
